...
class CentralCorridor(Scene):
	...
	def enter(self):
		print """
			...
			destroyed your entire crew. You are the last surviving member and
			your last mission is to get the neutron destruct bomb from the
		...
		if action == "shoot!":
			...
			print """
				...
				academy. You tell the one Gothon joke you know: Lbhe zbgure vf
				fb sng, jura fur fvgf nebhaq gur ubhfr, fur fvgf nebhaq gur
				ubhfr. The Gothon stops, tries not to laugh, then busts out
				laughing and can't move. While he's laughing you run up and
				shoot him square in the head putting him down, then jump
...
class LaserWeaponArmory(Scene):
	...
	def enter(self):
		print """
			...
			neutron bomb in its container. There's a keypad lock on the box and
			you need the code to get the bomb out. If you get the code wrong 10
			times then the lock closes forever and you can't get the bomb. The
			code is 3 digits.
		"""
	...
	def enter(self):
		...
		if action == "throw the bomb":
			print """
				...
				Gothon frantically try to disarm the bomb. You die knowing they
				will probably blow up when it goes off.
			"""
			return 'death'
			...
			print """
				...
				lock so the Gothons can't get out. Now that the bomb is placed
...
class EscapePod(Scene):
	...
	def enter(self):
		print """
			...
			get to the chamber with the escape pods, and now need to pick one
			to take. Some of them could be damaged but you don't have time to
		...
		if int(guess) != good_pod:
			print """
				You jump into pod %s and hit the eject button. The pod escapes
